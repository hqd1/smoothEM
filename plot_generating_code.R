...
plot10 = ggplot(sim_data_smooth, aes(x = grid, y = smooth_value))+
  geom_point(aes(x = grid, y = y, fill = spikes, shape = spikes), size = 1.4, stroke = 0)+
  ...
  scale_shape_manual(labels = c("flagged non-spike", "flagged spikes" ),values = c(21, 24))+
  scale_colour_manual(labels = c("fitted curve","true f"), values=c(true_smooth = jBrewColors[5], smooth = "#000099")) +
  scale_linetype_manual(labels = c("fitted curve","true f"), values = c(true_smooth = 1, smooth = 4))+
...
plot100 = ggplot(smooth_data, aes(x = grid, y = fit_value))+
  geom_point(aes(x = grid, y = y), fill = "grey69", color = "grey69", shape = 21, size = 1)+
  geom_line(aes(color = fit_id), size = 1)+
...
plot_grid(plot200,plot100, ncol = 2,rel_widths = c(0.43,0.57)) #999 by 500
...
plot_grid(pgrid, legend, nrow = 2, rel_heights = c(1, .1)) #850 by 745
...
set.seed(6)
sigma.jiggle = median(abs(heatwave$y - median(heatwave$y)))
#EM.result = classify(heatwave, range = c(1910,2015), norder = 4, Lcoef = c(0,1),lambda = c(50,10,1,1e-01,1e-02, 1e-03, 1e-05, 1e-08), smooth.true = NA, MSOM = TRUE, VIOM = TRUE, beta = 2, sigma.jiggle = sigma.jiggle)
EM.result = classify(heatwave, range = c(1910,2015), norder = 4, Lcoef = c(0,1),lambda = c(1000,500,100,50,10,5,1,1e-01,1e-02, 1e-03, 1e-04), smooth.true = NA, MSOM = TRUE, VIOM = TRUE, beta = 1, sigma.jiggle = sigma.jiggle)
smooth.data = fit(heatwave, range = c(1910,2015), norder = 4, Lcoef = c(0,1), spikes.flag = EM.result$EM.result$spikes.EM, lambda = EM.result$lambda, plot = FALSE)
spikes_flag = EM.result$EM.result$spikes.EM
...
sigma.jiggle = median(abs(summer_temp$y - median(summer_temp$y)))
EM.result = classify(summer_temp, range = c(1910,2015), norder = 4, Lcoef = c(0,1),lambda = c(50,10,1,1e-01,1e-02, 1e-03, 1e-05, 1e-08), smooth.true = NA, MSOM = TRUE, VIOM = TRUE, beta = 2, sigma.jiggle = sigma.jiggle)
spikes_flag = EM.result$EM.result$spikes.EM
...
######id 1977 ######
power_1977 = read.table("Data/38_CER Electricity_Gas/CER Electricity Revised March 2012/File1.txt", header = F) %>%
  rename(meter_id = V1, time = V2, power_consumption = V3) %>%
